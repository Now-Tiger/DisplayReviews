...
export default function DisplayReviews() {
  ...
  const dataNotFound = (
    <div className="mb-8 mt-7 max-w-xl rounded-xl overflow-hidden shadow-lg bg-slate-100">
      <div className="m-5">
          ...
          {data.length != 0 &&
            data.map(
              (item) =>
                ...
                item.analytics[0].category &&
                item.analytics[0].topic &&
                item.analytics[0].sentiment ? (
                  <li
                    className="mb-8 mt-7 max-w-xl rounded-xl overflow-hidden shadow-lg bg-slate-100"
                    key={item.review_id}
                  >
                    <h2 className="px-3 py-3 text-2xl font-semibold mb-3">
                      {item.reviewer_name}
                    </h2>
                    <div className="">
                      <p className="px-3 py-3">
                        <b className="text-emerald-700">Sentence</b>:{" "}
                        {item.analytics[0].sentences}
                      </p>
                      <p className="px-3 py-3 mb-3">
                        <b className="text-cyan-700">Sentiment</b>:{" "}
                        {item.analytics[0].sentiment}
                      </p>
                  ...
                  dataNotFound
                ), // If any required field is null, skip rendering the list item
            )}
        </ul>
      </main>
    </div>
